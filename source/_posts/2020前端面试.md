...
- 要判断一个运行中函数的`this绑定`，就需要找到这个函数的`直接调用位置`，最重要的就是`分析调用栈`（为了到达当前执行位置所调用的所有函数，即函数调用链）。
...
- ES6中`箭头函数()=>{}`并不会使用这四条标准的绑定规则，而是根据`当前的词法作用域`来决定this，具体来说，箭头函数会继承外层函数调用的this绑定（无论this绑定到什么）。这其实和ES6之前的`self = this`机制一样。
- 代码风格：
...
- ==[作用域链和原型链](https://cnodejs.org/topic/59662f6dbda29e0f7480235b)==
...
- [prototype、__proto__与constructor区别与联系](https://blog.csdn.net/qq_35376561/article/details/96651353)
    - function Person() {}
...
- [浅析JavaScript中原型及constructor、__proto__、prototype的关系](https://segmentfault.com/a/1190000019279667)
...
- [浅拷贝和深拷贝的区别](https://segmentfault.com/a/1190000018874254)
...
- 深拷贝的思路（递归？利用Object.prototype.toString.call(obj)判断类型，链接同上）
    - 递归方法实现深度克隆原理：遍历对象、数组直到里边都是基本数据类型，然后再去复制，就是深度拷贝
- [实现深拷贝的方法](http://www.javanx.cn/20191217/js-deep-copy/)
    ...
    - [proxy深拷贝](https://segmentfault.com/a/1190000021692975)
        ...
        - 判断我们深拷贝的对象是否有改变
- [函数声明和var声明的优先级](https://www.cnblogs.com/cnblogs-jcy/p/8926310.html)
- [如何利用正则匹配字符串](https://www.jb51.net/article/124041.htm)
    - exec：查找并返回当前的匹配结果，并以数组的形式返回；exec是RegExp对象的方法；每次只返回一次匹配项
    - match：math是String对象的方法；一次性返回所有匹配项
- [将字符串变成数字](https://www.cnblogs.com/mica/p/11760397.html)
...
- ~~new Set的数组去重和自己实现的哪个性能会更好~~
- [new操作符的作用及运行过程](https://blog.csdn.net/duanshilong/article/details/88235546)（new 运算符创建一个用户定义的对象类型的实例或具有构造函数的内置对象的实例）
...
- [Instanceof运算符的作用及运行过程](https://www.jianshu.com/p/5f6b4ec34059)（用于测试构造函数的prototype属性，是否出现在对象的原型链中的任何位置）
    - 判断构造函数的原型对象(如Person.prototype和Object.prototype)是否在实例对象（person1）的原型链上（__proto__）;如果在对象的原型链上，就返回true，如果不在就返回false;
- 变量提升是什么？Var、Const 、let 是如何变量提升的（暂时性死区）？
- [var与let、const的区别](https://www.jianshu.com/p/7bf685dbc12e)
    - var声明变量存在变量提升，let和const不存在变量提升
    - let、const都是块级局部变量
    - 同一作用域下let和const不能声明同名变量，而var可以
- [js检测变量是否已定义](https://blog.csdn.net/joyvonlee/article/details/89603278)
...
- [js判断对象中是否有某属性的常用方法](https://www.jb51.net/article/141994.htm)
...
- [ES6用过的新特性](https://juejin.im/post/6844903991550181390?utm_source=gold_browser_extension)
    - 箭头函数、字符串插值、const、let(块作用域)
    - Promises异步机制、模块import导入、模块export导出
    - function默认参数、class类定义与extend继承
    - for-of遍历对象、...展开操作符
- [es6中的数组方法](https://es6.ruanyifeng.com/#docs/array)
    - 扩展运算符(...)
    - Array.from()
    - Array.of()
    - 数组实例的 copyWithin()
...
- [es6中的对象方法](https://es6.ruanyifeng.com/#docs/object)
...
- [es6中的对象新增方法](https://es6.ruanyifeng.com/#docs/object-methods)
...
- [js es6遍历对象的6种方法（应用中推荐前三种）](https://www.cnblogs.com/yuer20180726/p/11377897.html)
...
- [forEach和for/for...of循环在使用await上的区别](https://blog.csdn.net/u012961419/article/details/107490076)，如果想同时执行呢？（promise.all）
...
- [IIEF立即执行为什么用两对括号包裹](https://www.jb51.net/article/127527.htm)
    - 模拟函数执行
...
- [`[]+{}`和`{}+[]`结果有何不同](https://blog.csdn.net/carcarrot/article/details/97786733)
...
- [var a=b=c=d=5是什么意思?如果接下来再写一句,d=9,a,b,c的值会变化吗?](https://www.axihe.com/focus/js/base/13.html)
    - 初始化给a、b、c、d赋值都为5
    - `不改变`，改变d后a、b、c值不会改变,因为a、b、c、d都是值类型的变量,各自的值存在于自己的栈当中,当d变化了其他栈中的值不改变。
- [var a=b=c=d=【1,2,3,4,5】是什么意思?如果接下来写一句d【5】=9;a,b,c,的值会发生变化吗](https://www.axihe.com/focus/js/base/14.html)
...
- [var a=b=c=d=【1,2,3,4,5】是什么意思?如果接下来再写一句d=【9】;a,b,c的值会发生变化吗](https://www.axihe.com/focus/js/base/15.html)
    - `不改变`,因为对于d来说改变的是d栈中的地址,此时d指向的堆已经不是原地址,所以此时d与其他几个对象的值已经不同了。
    - 考点：对比上一题,改变的是对应堆中的值,而此题是将d栈中的指针地址改变了。
- [js表达式和语句的区别](https://zhidao.baidu.com/question/1758450597152165508.html)
...
- [为什么会有跨域的问题？](https://www.jianshu.com/p/9b6b6a135432)
...
- [九种跨域方式实现原理](https://zhuanlan.zhihu.com/p/56718905)
...
- [JavaScript手动实现JSONP代码](https://www.cnblogs.com/gogolee/p/6439664.html)
...
- [对前端模块化的认识](https://blog.csdn.net/u014168594/article/details/77099315)
...
- [四种常见的JS模块化管理方法的比较(表格形式：导入语法、导出语法、加载方式等)](https://blog.csdn.net/Mrfive555/article/details/79668797?utm_medium=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-2.channel_param&depth_1-utm_source=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-2.channel_param)
- AMD 和 CMD 的区别
...
- ~~XML和JSON的区别？~~
- [什么是dom0，dom1，dom2，dom3](https://zhidao.baidu.com/question/1771299590043658140.html)
    - DOM0：不是W3C规范。
    - DOM1：开始是W3C规范。专注于HTML文档和XML文档。
    - DOM2：对DOM1增加了样式表对象模型
    - DOM3：对DOM2增加了内容模型(DTD、Schemas) 和文档验证。
- [dom0级和dom2级的区别](https://www.imooc.com/qadetail/168609)
    - DOM0级事件处理：简单且具有跨浏览器的优势；但函数会被覆盖，只会执行最后一个的函数
    - DOM2级事件处理：不会覆盖且顺序执行；不具有跨浏览器优势
- [事件，事件冒泡和事件捕获](https://www.cnblogs.com/Shinigami/p/10054696.html)
- [addEventListener的第三个参数到底该怎么设置](https://www.jianshu.com/p/bad857d649f2)
...
- [Web Worker](http://www.ruanyifeng.com/blog/2018/07/web-worker.html) 的作用就是为 JavaScript 创造多线程环境，允许主线程创建 Worker 线程，将一些任务分配给后者运行。在主线程运行的同时，Worker线程在后台运行，两者互不干扰。等到Worker线程完成计算任务，再把结果返回给主线程。这样的好处是，一些计算密集型或高延迟的任务，被Worker线程负担了，主线程（通常负责UI交互）就会很流畅，不会被阻塞或拖慢。
...
- [js按位运算符](https://www.cnblogs.com/happy1992/p/7064114.html)
    - `&` 按位与
    - `|` 按位或
    - `^` 按位异或
    - `~` 按位非
    - `>>` 右移
    - `<<` 左移
- [常见的js位运算](https://www.jianshu.com/p/768f50569130)
...
∴ 2 & 1 = 0
```
- [2..toString](https://blog.csdn.net/newair1798/article/details/88207671)
...
- [数组的降维5种办法](https://blog.csdn.net/xufeiayang/article/details/90111775)
    - 数组字符串化：`arr += ''; arr = arr.split(',');`
    - 递归，判断后push或concat
    - `Array.prototype.flat()`
    - 使用reduce、concat和递归无限反嵌套多层嵌套的数组
- [数组的join](https://www.w3school.com.cn/jsref/jsref_join.asp)：把数组中的所有元素拼接成一个字符串，元素是通过指定的分隔符进行分隔的。
- [字符串的split](https://www.w3school.com.cn/jsref/jsref_split.asp)：把一个字符串通过指定的分隔符分割成字符串数组。
- [数组的map和forEach的区别](https://www.jianshu.com/p/83aa9a2a4055)
...
- [js如何判断数据类型](https://www.cnblogs.com/yi0921/p/6183422.html)
...
- [ES6中Promise.all和Promise.race区别](https://www.cnblogs.com/richard1015/p/9155564.html)
...
- [js如何操作cookie](https://www.jianshu.com/p/cf557cfe460c)
    ...
    - 或 利用小框架：`cookies.js`
- [Cookie和Session的使用和区别](https://www.jianshu.com/p/9a561b36e9f3)
- [js进程（process）和线程（thread）的区别](https://zhidao.baidu.com/question/1308689490865592819.html)
    - 进程：操作系统分配的占有CPU资源的最小单位。拥有独立的地址空间。
    - 线程：安排CPU执行的最小单位。同一个进程下的所有线程，共享进程的地址空间。
- [js进程（process）和线程（thread）的关系](https://www.cnblogs.com/yu-hailong/p/9596431.html)
    - 以多进程形式，允许多个任务同时运行
...
- [Object.create实现了什么？传null得到的结果和普通对象有什么区别？](https://www.jianshu.com/p/28d85bebe599)
    - 创建对象的方式不同
    ...
    - `__proto__`属性
        - newObject():？
        - Object.create():？
- [call、apply和bind有何区别](https://www.cnblogs.com/cosiray/p/4512969.html)
...
- [手写实现call、apply、bind](https://www.jianshu.com/p/57a876fe66c8)
...
```
- [call、apply、bind的应用](https://www.jianshu.com/p/bc541afad6ee)
...
- 实现请求并发限制，具体为：封装一个函数，传递请求并发的个数为参数，实现对并发请求的限制
- 说说闭包以及垃圾回收机制
- 利用async和await如何处理异常事件
- 箭头函数和普通函数有什么区别？如果想改变箭头函数中绑定this怎么办？
- 原生js判断鼠标在一个有对角线矩形的位置
...
- [vue.js的两大核心](https://www.cnblogs.com/qlongbg/p/13025274.html)
    - 数据驱动，也就是数据的双向绑定
    - 组件系统
...
- [什么是MVVM](https://juejin.im/entry/6844903491886907399)（操作数据，就是操作视图，就是操作DOM）
- MVVM的优点:
...
- [你对Vue3.0的新特性有没有了解?](https://www.yuque.com/woniuppp/vue3/feature)
- vue2和vue3的特点和新特性
...
- 双向绑定的原理（差点让我手撸一个双向绑定）[Object.defineProperty()](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Object/defineProperty)和[Proxy](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Proxy)
...
- 既然Vue通过数据劫持可以精准探测数据变化，为什么还需要虛拟DOM进行diff检测差异?
    - 考点: Vue的变化侦测原理
...
- [简单说一说vue生命周期的理解?](https://www.axihe.com/focus/vue/08.html)
...
- [Vue.js的八大生命周期](https://www.axihe.com/focus/vue/08.html)
- [Vue.js的父子组件生命周期调用顺序](https://www.axihe.com/focus/vue/23.html)
- [Vue.js的ajax请求放在哪个生命周期中](https://www.axihe.com/focus/vue/09.html)
- 你知道nextTick的原理吗?
...
- 如果改了data的多个值update钩子会执行几次，为什么

> computed && watch

- Vue computed 的实现原理
- Vue watch 的实现原理
- computed和watch的区别和应用


> vuex

- [vuex是什么](https://vuex.vuejs.org/zh/)、[vuex的源码](https://unpkg.com/vuex@3.5.1/dist/vuex.js)、底层（Vuex的注入代码比较简单,调用了一下applyMixin方法,现在的版本其实就是调用了Vue）、使用场景（组件通信、登录状态、加入购物车、公共码表维护、页面缓存(慎用)）
- [mutation的用法](https://vuex.vuejs.org/zh/guide/mutations.html)

> vue-router

- Vue-router 的实现原理
- vue的两种路由模式，问我这两种路由模式的底层实现（昨天刚复习过 美滋滋）
- vue-router中push和replace的区别
- [vueRouter的钩子函数](https://zhuanlan.zhihu.com/p/70536937)
    - 全局的路由钩子函数：beforeEach（全局前置守卫）、afterEach（全局后置守卫）
    - 路由配置文件独享的钩子函数：beforeEnter
    - 组件内的路由钩子函数：beforeRouteEnter、beforeRouteLeave、beforeRouteUpdate
- [vue-router中的导航钩子由那些?](https://zhuanlan.zhihu.com/p/70536937)
    - 全局的路由钩子函数：beforeEach（全局前置守卫）、afterEach（全局后置守卫）
    - 路由配置文件独享的钩子函数：beforeEnter
    - 组件内的路由钩子函数：beforeRouteEnter、beforeRouteLeave、beforeRouteUpdate
- 如何（动态）控制路有权限，颗粒多大
...
- [Vue.js组件如何通信以及有哪些方式?](https://www.axihe.com/focus/vue/24.html)
- vue中如何实现兄弟组件通信：
...
- [v-on事件修饰符及按键修饰符](https://blog.csdn.net/sleepwalker_1992/article/details/82903246)
    - `.stop`：阻止事件冒泡
...
- [vue自定义指令和私有指令](https://cn.vuejs.org/v2/guide/custom-directive.html)
- [delete和Vue.delete(this.$delete)删除数组的区别](https://www.jianshu.com/p/fbe96ed71fd7)
    - var a=[1,2,3,4]
    - var b=[1,2,3,4]
    - delete a[1]
    ...
    - console.log(b) // [1,3,4]
- v-model的作用和原理
- v-show和v-if的区别，与dom操作是否相关
- 如何取到子组件的实例，除了$ref还有无别的方法

> others

- vue中css处scoped的实现原理及穿透的用法
- [video的层级](https://www.jianshu.com/p/79100d43939a)（他的层级是最高的，问我怎么在video上添加一些div，我之前有看过aliplayer这个插件，面试官也很满意）
- [30行写一个Vue图片懒加载指令](https://mp.weixin.qq.com/s/vtg6DaGKdslHQZUTpDiC7w)
...
# 实战
编程：编译一下模板字符串
编程：实现音频和动画的onStart和onEnd异步控制
编程：手动实现new运算符
编程：手动实现instanceof运算符
编程：手写parseInt
